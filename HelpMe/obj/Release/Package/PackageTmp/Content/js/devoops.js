...
function LoadTimePickerScript(callback) {
    if (!$.fn.timepicker) {
        $.getScript('Content/plugins/jquery-ui-timepicker-addon/jquery-ui-timepicker-addon.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadBootstrapValidatorScript(callback) {
    if (!$.fn.bootstrapValidator) {
        $.getScript('Content/plugins/bootstrapvalidator/bootstrapValidator.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
function LoadSelect2Script(callback) {
    if (!$.fn.select2) {
        $.getScript('Content/plugins/select2/select2.min.js', callback);
    }
    else {
        if (callback && typeof (callback) === "function") {
            callback();
...
$(document).ready(function () {
    ...
    $('#search').on('keydown', function (e) {
        if (e.keyCode == 13) {
            e.preventDefault();
            $('#content').removeClass('full-content');
            ajax_url = 'searchrecord.aspx';      //ajax/page_search.html
    ...
    $('#screen_unlock').on('mouseover', function () {
        ...
        var button = $('<div class="text-center"><a href="Dashboard.aspx" class="btn btn-primary">Unlock</a></div>');
