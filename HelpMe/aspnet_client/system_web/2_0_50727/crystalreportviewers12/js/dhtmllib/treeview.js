...
// returns [String] the HTML
{
	...
	with (this)
	{
		...
		var onkeydown='onkeydown="'+_codeWinName+'._tkl(this,event)" ';
		var onclick='onclick="'+_codeWinName+'._tpl(this,event)" ';
		            
		if (exp)
		{
		    // TODO: Post Titan... Justin.  The two strings used below are in the Viewer's string_xx.js files.
		    // This is generic enough functionality that its possible to move to the DHTML labels.js.  We CANNOT
		    // continue to have the DHTMLLib dependant on the Viewer's resources.
		    var expIcon;
		    var iconTooltip;
		    if(expanded)
		    {
		        ...
		        iconTooltip =  L_bobj_crv_Tree_Collapse_Node.replace('%1', name);
		    }
		    else
		    {
		        ...
		        iconTooltip =  L_bobj_crv_Tree_Expand_Node.replace('%1', name);
		    }
			a[i++]='<img tabindex=1 '+onclick + ' ' + onkeydown +attr('title', iconTooltip) + 'class=trPlus src="'+expIcon+'"/>'
		}
		...
		a[i++]='<span '+mouseCB+ ' ' + keyCB + 'tabindex=1'+acceptDD+' class='+nonselectedClass+'>'
		...
		a[i++]=(iconId>-1?imgOffset(treeView.icns,treeView.iconW,treeView.iconH,(treeView.iconOrientVertical?0:treeView.iconW*(expanded?iconSelId:iconId)),(treeView.iconOrientVertical?treeView.iconH*(expanded?iconSelId:iconId):0),null,null,iconAlt,exp?'':mrg,'top'):'')
		a[i++]='</span>'
