 ...
 */
bobj.isSafari = function() {
    var appVersion=navigator.appVersion.toLowerCase();
    return (appVersion.indexOf('macintosh')>=0)||(appVersion.indexOf('macos')>=0);
};
...
bobj.setOuterSize = function(node, w, h, excludeMargins) {
    ...
    if (bobj.isNumber(w)) {
        if( bobj.isSafari() === false ) {
            if (!excludeMargins) {
                w -= pixels(node.style.marginLeft);
                w -= pixels(node.style.marginRight);
            }
            if (!bobj.isBorderBoxModel()) {
                w -= pixels(node.style.borderLeftWidth);
                w -= pixels(node.style.borderRightWidth);
                w -= pixels(node.style.marginLeft);
                w -= pixels(node.style.marginRight);
                w -= pixels(node.style.paddingLeft);
                w -= pixels(node.style.paddingRight);
            }
        }
    ...
    if (bobj.isNumber(h)) {
        if(bobj.isSafari() === false) {
            if (!excludeMargins) {
                h -= pixels(node.style.marginTop);
                h -= pixels(node.style.marginBottom);
            }
                
            if (!bobj.isBorderBoxModel()) {
                h -= pixels(node.style.borderTopWidth);
                h -= pixels(node.style.borderBottomWidth);
                h -= pixels(node.style.paddingTop);
                h -= pixels(node.style.paddingBottom);
            }
        }        
