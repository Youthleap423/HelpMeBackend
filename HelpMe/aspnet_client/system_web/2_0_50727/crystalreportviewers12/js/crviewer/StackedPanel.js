...
bobj.crv.StackedTab.getHTML = function() {
    ...
    var html = DIV({id: this.id, style: ctnStyle},
        DIV(labelCtnAtt,
            DIV(expandCtnAtt, 
                imgOffset(bobj.crvUri('images/StackedTab.gif'), imgW, imgH, imgW, 0, this.id + '_expand')),
...
bobj.crv.StackedTab._getIconsHtml = function(icons, isLeftAligned) { 
    ...
    for (var i = 0, len = icons.length; i < len; ++i) {
        ...
        iconsHtml += DIV({id: icon.id, 'class': 'stackedTabIconCtn', style: this._getIconCtnStyle(icon, isLeftAligned, i)}, 
            imgOffset(icon.url, this._IMG_WIDTH, this._IMG_HEIGHT, 0, 0, null, null, (isInfoIcon? null : icon.tooltip)));    
    }
 ...
 */
bobj.crv.StackedTab.addIcon = function(url, tooltip, isLeftAligned, isVisible, id) {
    id = id || bobj.uniqueId();
    var iconInfo = {
        url: url,
        tooltip: tooltip,
...
bobj.crv.StackedTab.resize = function(w) {
    if(this._labelCtn) {
        bobj.setOuterSize(this._labelCtn, w);   
    }
    if (this._content) { 
...
bobj.crv.StackedTab.expand = function() {
    if (!this.isExpanded()) {
        changeOffset(this._expandImg, 16, 0);
...
bobj.crv.StackedTab.collapse = function() {
    changeOffset(this._expandImg, 0, 0);
